...
const textureLoader = new THREE.TextureLoader(); // Add this line
...
export function createFerry() {
    ...
    // --- Load Textures ---
    const leftTexture = textureLoader.load('images/ferry_left.png');
    const rightTexture = textureLoader.load('images/ferry_right.png');
    const frontTexture = textureLoader.load('images/ferry_front.png');
    const backTexture = textureLoader.load('images/ferry_back.png');
    const topTexture = textureLoader.load('images/ferry_top.png'); // Load the top texture again
    // NEW: Load the specific cabin back texture
    const cabinBackTexture = textureLoader.load('images/cabin_back.png'); // Add your cabin back texture file

    // --- Materials ---
    const sideMaterialOptions = {
        metalness: 0.1,
        roughness: 0.9,
        transparent: true // Set true if PNGs have transparency
    };
    ...
    const topMaterial = new THREE.MeshStandardMaterial({ map: topTexture, ...sideMaterialOptions }); // Create material for top texture
    // NEW: Create the specific cabin back material
    const cabinBackMaterial = new THREE.MeshStandardMaterial({ map: cabinBackTexture, ...sideMaterialOptions });


    // Fallback/base materials for untextured parts (only bottom is needed now)
    /* REMOVED fallbackTopMaterial
    const fallbackTopMaterial = new THREE.MeshStandardMaterial({
        color: 0xffffff,
        metalness: 0.2,
        roughness: 0.8
    });
    */
    const bottomMaterial = new THREE.MeshStandardMaterial({
        color: 0xcccccc, // Light grey for bottom
        metalness: 0.2,
        roughness: 0.8
    });

    // Material for the antenna and potentially deck sides/top
    ...
    // --- 1. Main Hull (Box with specific materials per face) ---
    const hullGeom = new THREE.BoxGeometry(hullWidth, hullHeight, hullLength);

    // Order of materials for BoxGeometry faces:
    // 0: right (+x), 1: left (-x), 2: top (+y), 3: bottom (-y), 4: back (+z), 5: front (-z)

    const hullMaterials = [
        rightMaterial,  // 0: right
        leftMaterial,   // 1: left
        topMaterial,    // 2: top <-- CORRECTED: Use loaded top texture
    ...
    const cabinLength = hullLength * 0.3;
    const cabinGeom = new THREE.BoxGeometry(cabinWidth, cabinHeight, cabinLength);

    const cabinMaterials = [
        rightMaterial,      // 0: right
        leftMaterial,       // 1: left
        topMaterial,        // 2: top (Use main top texture, or create a specific cabin top)
        bottomMaterial,     // 3: bottom (Covered)
        cabinBackMaterial,  // 4: back <-- CORRECTED: Use specific cabin back texture
        frontMaterial       // 5: front
    ];
...
// ... (keep createWater, createSky, createCheckpoints, createParticles, updateGraphics, emitWakeParticle, emitBowParticle functions as they were in the version you provided) ...
...
// NOTE: Using the simple sphere particles from the version you provided
...
// ----- UPDATE FUNCTIONS -----
// NOTE: Using the simple particle animation from the version you provided
...
// ----- PARTICLE EMITTERS -----
// NOTE: Using the simple particle emitters from the version you provided
